...
func _ready() -> void:
	...
	#var height = int(get_viewport().get_visible_rect().size[1]/2)-50
...
func set_variables(a_dict) -> void:
	...
	if type == "Tree-test":
		...
		num_steps_down = 45.0
		num_steps_up = 35.0
		alpha_step_down = (high_alpha - low_alpha) / num_steps_down
		alpha_step_up = (high_alpha - low_alpha) / num_steps_up

		scale_step_down = (max_scale - min_scale) / num_steps_down
		scale_step_up = (max_scale - min_scale) / num_steps_up
